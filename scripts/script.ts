...
let data: { fraction: Element[]; fractionOf: Element[] };
...
const init = async () => {
	...
	if (data.fraction.length === 0 && data.fractionOf.length === 0) {
		errorVisualisation();
	} else {
		updateVisualisation(data.fraction.length, data.fractionOf.length);
	}
};

const changeArchive = async () => {
	data = await getArchive();
